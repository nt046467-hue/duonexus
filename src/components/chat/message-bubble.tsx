...
import { Check, CheckCheck, Trash2, MoreVertical, Play, Pause, Smile, Download, Reply, Phone, Video, PhoneMissed } from "lucide-react";
...
export function MessageBubble({
  ...
  const isCallLog = type === "text" && (
    content.startsWith("📞") ||
    content.startsWith("📹") ||
    content.startsWith("🎥") ||
    content.toLowerCase().startsWith("[audio call") ||
    content.toLowerCase().startsWith("[video call") ||
    content.toLowerCase().includes(" called ") ||
    (content.toLowerCase().includes("call") && content.toLowerCase().includes("min"))
  );

  if (isCallLog) {
    const isMissed = content.toLowerCase().includes("missed") || content.toLowerCase().includes("declined");
    const isVideo = content.startsWith("📹") || content.startsWith("🎥") || content.toLowerCase().includes("video");
    // Strip leading emoji for clean text display
    ...
    return (
      ...
      >
        <div className={cn(
          "px-4 py-2 rounded-2xl text-[12px] font-medium flex items-center gap-2.5 border shadow-sm backdrop-blur-sm select-none max-w-[260px]",
          isMissed
          ...
          <div className={cn(
            ...
            isMissed ? "bg-red-500/20" : "bg-emerald-500/20"
          )}>
            {isMissed ? (
          ...
          <div className="flex flex-col min-w-0">
            <span className="font-semibold text-[12px] leading-tight truncate">{cleanContent}</span>
            <span className="opacity-60 text-[10px] font-sans mt-0.5">
