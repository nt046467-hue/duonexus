...
interface ChatHeaderProps {
  ...
  isOwnProfileOpen?: boolean;
  onOwnProfileOpenChange?: (open: boolean) => void;
}
...
export function ChatHeader({
  ...
  isOwnProfileOpen: externalOwnProfileOpen,
  onOwnProfileOpenChange,
}: ChatHeaderProps) {
  ...
  const isSettingsOpen = externalSettingsOpen !== undefined ? externalSettingsOpen : internalSettingsOpen;
  ...
  const [internalOwnProfileOpen, setInternalOwnProfileOpen] = useState(false);
  const isOwnProfileOpen = externalOwnProfileOpen !== undefined ? externalOwnProfileOpen : internalOwnProfileOpen;
  const setIsOwnProfileOpen = (open: boolean) => {
    setInternalOwnProfileOpen(open);
    onOwnProfileOpenChange?.(open);
  };
  ...
  const typingRef = useMemoFirebase(() => {
    if (!db || !partnerId || !user) return null;
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      await setDoc(
        ...
        {
          displayName: newName,
          photoURL: newPhotoURL || (myId === "nabin" ? "/avatars/nabin.png" : "/avatars/karu.png"),
          lastActive: serverTimestamp(),
        },
        { merge: true }
      );
      await updateProfile(user, { displayName: newName });
      toast({ title: "Profile Updated", description: "Changes live! ❤️" });
          ...
          <div className="relative shrink-0">
            ...
            >
              <Avatar className="h-10 w-10 border-2 border-primary/20 shadow-sm bg-black">
                <AvatarImage key={partnerAvatar} src={partnerAvatar} className="rounded-full object-cover bg-black" />
