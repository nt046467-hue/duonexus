...
export function WallpaperFramingViewport({
  ...
  return (
    <div className={cn("flex flex-col items-center w-full select-none", className)}>
      {/* ── DEVICE MOCKUP CONTAINER (PHYSICAL BEZEL / CHASSIS) ── */}
      <div
        className={cn(
          "transition-all duration-300 flex flex-col items-center justify-center shrink-0",
          isPhone
            ? "w-[245px] sm:w-[272px] aspect-[9/18.8] p-[8px] sm:p-[9px] rounded-[46px] sm:rounded-[50px] bg-[#1a1b1e] ring-1 ring-white/20 border border-zinc-800 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.85),0_0_0_1px_rgba(255,255,255,0.08)]"
            : "w-full aspect-[16/10] max-h-[440px] p-[6px] rounded-3xl bg-zinc-950 ring-1 ring-primary/15 border border-border/50 shadow-2xl"
        )}
      >
        {/* ── UNIFIED INNER SCREEN VIEWPORT (SINGLE SOURCE OF TRUTH FOR CURVATURE & CLIPPING) ── */}
        <div
          className={cn(
            "relative w-full h-full overflow-hidden isolate flex flex-col justify-between bg-zinc-950",
            isPhone
              ? "rounded-[38px] sm:rounded-[41px]"
        ...
        >
          {/* Dynamic Island / Hardware Sensors for Phone Mode */}
          {isPhone && (
            <div className="absolute top-2 inset-x-0 z-30 flex justify-center pointer-events-none">
              <div className="w-[78px] h-[19px] bg-black rounded-full border border-white/10 flex items-center justify-between px-2.5 shadow-md">
                <div className="w-2.5 h-2.5 rounded-full bg-zinc-900 border border-blue-500/30 flex items-center justify-center">
                  <div className="w-1 h-1 rounded-full bg-blue-600/40" />
                </div>
                <div className="w-2 h-2 rounded-full bg-zinc-900/90 border border-white/5" />
              </div>
            </div>
          )}
          ...
          <div
            className={cn(
              ...
              isDark
                ? "bg-[#18181A]/85 border-white/10 text-white"
                : "bg-white/85 border-black/10 text-gray-900",
              isPhone ? "pt-7 pb-2 px-3.5" : "py-2.5 px-4"
            )}
          >
          ...
          <div className="relative z-20 pointer-events-none flex-1 flex flex-col justify-end p-3 space-y-2 overflow-hidden">
            ...
            <div className="max-w-[82%] flex flex-col items-start space-y-0.5">
              <div
                className={cn(
            ...
            <div className="max-w-[82%] ml-auto flex flex-col items-end space-y-0.5">
              <div className="rounded-2xl rounded-tr-xs px-3 py-1.5 shadow-sm text-[11px] font-medium leading-relaxed bg-[#1877F2] text-white shadow-xs">
          ...
          <div
            className={cn(
              ...
              isDark
                ? "bg-[#18181A]/85 border-white/10"
                : "bg-white/85 border-black/10",
              isPhone ? "pb-2 pt-2 px-3" : "py-2 px-3"
            )}
          >
            ...
            {/* Home Indicator Bar on Phone screen */}
            {isPhone && (
              <div className="w-20 h-1 bg-white/40 rounded-full mx-auto mt-1.5 mb-0.5" />
            )}
          </div>
        </div>
      </div>
